---
name: website-building/informational
description: Build distinctive informational websites — personal sites, portfolios, editorial publications, blogs, small business pages, and landing pages. Covers art direction, typography, white space, multi-column editorial layouts, SEO, and performance.
---
...
- **Content images must be real.** Use the `url` from `search_images` results directly in `<img>` tags — never hallucinate image URLs. Use `generate_image` for custom hero images and illustrations. Every `<img>` must include `alt`, `width`, `height`, and `loading="lazy"`. When neither search results nor generated images are available, use Lorem Picsum (`https://picsum.photos/seed/{topic}/{w}/{h}`) rather than leaving gaps.
...
On mobile, all multi-column layouts collapse to a single column — but maintain visual rhythm by keeping full-bleed images and pull quotes as section dividers. See the Layout section in `skills/website-building/SKILL.md` for grid implementation.
...
- **Restraint IS the identity.** Personality comes from typography and spacing, not color or effects. (AI aesthetic anti-patterns in the Standards section in `SKILL.md`.)
...
Beyond baseline in the Standards section in `skills/website-building/SKILL.md`:
...
Ask: site type? audience? primary action? tone? brand assets? content scope? reference sites?

**Default (minimal input):** Satoshi or General Sans (Fontshare), generous whitespace, grid-aligned, Nexus palette, 1 accent. Editorial: serif display (Boska/Zodiak). See "The Fallback: Clean & Swiss" in `SKILL.md`.

**Key shared files:** `shared/04-motion.md` (scroll effects, GSAP SVG), `shared/09-toolkit.md` (SVG patterns/filters).
